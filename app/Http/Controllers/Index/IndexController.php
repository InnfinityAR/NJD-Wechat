...
class IndexController extends CommonController {
    ...
    public function index() {
        ...
        $educations = Article::where("nav_id", $education_id)->limit(12)->get();
        
        // 留学资讯
        $abord_id = Nav::where("name", "留学资讯")->pluck("id")->first();
        $abords = Article::where("nav_id", $abord_id)->limit(12)->get();
        
        // TDK
        $desc = $this->web['desc'];
        $keywords = $this->web['keywords'];
        
        return view("index.index.index", compact("contacts", "teachers", "systems", "exams", "educations", "abords", "desc", "keywords"));
    }
    ...
    public function map() {
        ...
        $top_nav_map["pid"] = 0;
        $top_nav_map["position"] = 1;       // 顶部
        $top_navs = Nav::where($top_nav_map)->orderBy("sort", "asc")->get()->toArray();
        foreach ($top_navs as $key => $top_nav) {
            $sub_nav_map["top_pid"] = $top_nav["id"];
            $sub_navs = Nav::where($sub_nav_map)->get();
            foreach ($sub_navs as $k=>$sub_nav){    // 去除导航中不可点击的部分
                if($sub_nav['level'] == 2 && hasChildNav($sub_nav["id"])){
                    unset($sub_navs[$k]);
                }
            }
            $top_navs[$key]["sub_navs"] = $sub_navs;
        }
        ...
        return view("index.index.map", compact("top_navs", "hot_tips", "contacts", "right_navs", "title", "desc", "keywords"));
    }
    ...
    public function link() {
        ...
        return view("index.index.link", compact("cooperations", "hot_tips", "contacts", "right_navs", "title", "desc", "keywords"));
    }
    ...
    public function saveUser(Request $request) {
        ...
        if($res){
            ...
            if(isset($input['age'])){
                $input["province"] = getField($input["province"], "area");
                $input["city"] = getField($input["city"], "area");
                $postFields["sex"] = $input["sex"];
                $postFields["abroad_state"] = $input["nation"];
                $postFields["other"] = "当前学历:".$input["education"].";所在省份:".$input["province"].";所在城市:".$input["city"].";QQ号码:".$input['qq'].";提问内容:".$input["content"];
            }
